...
class TestUtils(TestCase):
    ...
    def test_get_shapefile(self):
        ...
        Test get_shapefile
        """
        path = os.path.join(BASE_DIR, "tests", "fixtures", "test_shapefile.zip")
        ...
        path = os.path.join(BASE_DIR, "tests", "fixtures", "SamburuCentralPolygon.zip")
        ...
        self.assertEqual(
            get_shapefile(zip_file6), "SamburuCentralPolygon/mytestfile.shp"
        )
    ...
    def test_get_allowed_contenttypes(self):
        ...
        rule_type = ContentType.objects.get(app_label="tasking", model="segmentrule")

        allowed = get_allowed_contenttypes(allowed_content_types=input_expected)
    ...
    def test_generate_task_occurrences(self):
        ...
        task2 = mommy.make(
            "tasking.Task", timing_rule="RRULE:FREQ=DAILY;INTERVAL=10;COUNT=5000"
        )
        ...
        for item in occurrences1:
            self.assertEqual(
                ...
                task1.start.astimezone(timezone.get_current_timezone()).time().hour,
            )
            self.assertEqual(
                item.start_time.minute,
                task1.start.astimezone(timezone.get_current_timezone()).time().minute,
            )
    ...
    def test_get_occurrence_end_time(self):
        ...
        self.assertEqual(
            "21:00:00",
            get_occurrence_end_time(task, the_rrule, end_time_input=None).isoformat(),
        )
        ...
        self.assertEqual(
            "19:15:00",
            get_occurrence_end_time(
                task, the_rrule, end_time_input=time(19, 15, 0, 0)
            ).isoformat(),
        )
        ...
        self.assertEqual(
            "23:59:59.999999",
            get_occurrence_end_time(task, the_rrule2, end_time_input=None).isoformat(),
        )
    ...
    def test_get_polygons_nested(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            multipolygon_list = [
                _ for _ in geom_object_list if isinstance(_, geometries.MultiPolygon)
            ]
    ...
    @override_settings(TASKING_SHAPEFILE_IGNORE_INVALID_TYPES=True)
