...
class TestLocationTypeViewSet(TestCase):
    ...
    def test_delete_location_type(self):
        ...
        self.assertTrue(LocationType.objects.filter(pk=locationtype.id).exists())

        view = LocationTypeViewSet.as_view({"delete": "destroy"})
        request = self.factory.delete(f"/locationtypes/{locationtype.id}")
        force_authenticate(request, user=user)
        response = view(request=request, pk=locationtype.id)

        self.assertEqual(response.status_code, 204)
        self.assertFalse(LocationType.objects.filter(pk=locationtype.id).exists())
    ...
    def test_retrieve_location_type(self):
        ...
        request = self.factory.get(f"/locationtypes/{locationtype_id}")
        force_authenticate(request, user=user)
    ...
    def test_update_location_type(self):
        ...
        request = self.factory.patch(f"/locationtypes/{locationtype_id}", data=data)
        force_authenticate(request, user=user)
        response = view(request=request, pk=locationtype_id)
    ...
    def test_authentication_required(self):
        ...
        request2 = self.factory.get(f"/locationtypes/{locationtype_id}")
        ...
        request4 = self.factory.delete(f"/locationtypes/{locationtype.id}")
        ...
        request5 = self.factory.patch(f"/locationtypes/{locationtype_id}", data=data)
