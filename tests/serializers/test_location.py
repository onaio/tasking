...
from tasking.common_tags import GEODETAILS_ONLY, GEOPOINT_MISSING, RADIUS_MISSING
...
class TestLocationSerializer(TestCase):
    ...
    def test_location_serializer_validate_shapefile(self):
        ...
        data = OrderedDict(
            name="Montreal", shapefile=mocked_location_with_shapefile.shapefile
        )
    ...
    def test_location_serializer_validate_geodetails(self):
        ...
        geopoint and radius
        """
        data = OrderedDict(name="Spain", geopoint="30,10", radius=45.986,)
        validated_data = LocationSerializer().validate(data)
        self.assertDictEqual(dict(data), dict(validated_data))
    ...
    def test_geopoint_field_output(self):
        ...
        self.assertTrue(serializer_instance.is_valid())
        self.assertEqual(type(serializer_instance.data["geopoint"]), GeoJsonDict)
    ...
    def test_shapefile_field_output(self):
        ...
        with open(path, "r+b") as shapefile:
            data = OrderedDict(name="Nairobi", country="KE", shapefile=shapefile)
            serializer_instance = LocationSerializer(data=data)

            self.assertTrue(serializer_instance.is_valid())
            self.assertEqual(type(serializer_instance.data["shapefile"]), GeoJsonDict)
    ...
    def test_location_create_given_binary_shapefile_data(self):
        ...
        sorted_key_list = [
            "{}".format(y) for y in sorted([int(x) for x in shapefile_keys_list])
        ]
    ...
    def test_bad_shapefile_data(self):
        ...
        with open(missing_files_path, "r+b") as shapefile:
            ...
            self.assertEqual(
                ...
                {
                    "shapefile": [
                        ErrorDetail(string=MissingFiles().message, code="invalid")
                    ]
                },
            )
        ...
        shapefile_not_found_files_path = os.path.join(
            BASE_DIR, "fixtures", "missing_shp.zip"
        )
        ...
        with open(shapefile_not_found_files_path, "r+b") as shapefile:
            ...
            self.assertEqual(
                ...
                {
                    "shapefile": [
                        ErrorDetail(string=ShapeFileNotFound().message, code="invalid")
                    ]
                },
            )
            ...
            with open(unnecessary_files_path, "r+b") as shapefile:
                ...
                self.assertEqual(
                    serializer_instance.errors,
                    {
                        "shapefile": [
                            ErrorDetail(
                                string=UnnecessaryFiles().message, code="invalid"
                            )
                        ]
                    },
                )
    ...
    def test_invalid_shapefile(self, mock):
        ...
        with open(path, "r+b") as shapefile:
            ...
            self.assertEqual(
                ...
                {
                    "shapefile": [
                        ErrorDetail(string=InvalidShapeFile().message, code="invalid")
                    ]
                },
            )
